...
(defn patient-list []
  (let [patients @(re-frame/subscribe [::subs/patients])
        ...
        active-patient (->> patients
                            ...
                            first)]
    (when (every? (complement nil?) [patients cur-location cur-patient])
      [:ul.nav.flex-column
...
(defn visits-list []
  (let [visits @(re-frame/subscribe [::subs/visits])
        ...
        active-visit (->> visits
                          ...
                          (first))]
    (when (every? (complement nil?) [visits cur-location cur-patient cur-visit])
...
(defn exp-list []
  (let [exps @(re-frame/subscribe [::subs/exps])
        ...
        cur-patient @(re-frame/subscribe [::subs/cur-patient])
        cur-visit @(re-frame/subscribe [::subs/cur-visit])]
    (when (every? (complement nil?) [exps cur-location cur-patient cur-visit])
...
(defn show-exp [exp-id]
  (let [exp @(re-frame/subscribe [::subs/exp-by-id exp-id])
        ...
        cur-patient @(re-frame/subscribe [::subs/cur-patient])
        cur-visit @(re-frame/subscribe [::subs/cur-visit])]
    (when (every? (complement nil?) [exp cur-location cur-patient cur-visit])
      [:div
       [:a.h6.align-left
        {:href (router/url-for :list-exps
                               :location-name cur-location
